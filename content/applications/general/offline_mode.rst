...
When the internet connection is no longer active, any views or records that were previously
opened while online remain accessible; previously opened records can be viewed but not modified
while offline.
...
.. note::
   - Odoo supports offline access for apps and modules using standard Odoo framework components.
     Offline access cannot be guaranteed for apps or modules using custom components.
